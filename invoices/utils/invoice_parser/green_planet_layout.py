...
def parse(text: str) -> dict:
    ...
    # Tìm ngày hóa đơn với nhiều fallback
    match_date = re.search(r"ngay (\d{1,2}) thang (\d{1,2}) nam (\d{4})", text)
    if not match_date:
        match_date = re.search(r"ngay[^\d]{0,15}(\d{1,2})[^\d]{0,15}(\d{1,2})[^\d]{0,15}(\d{4})", text)
    if not match_date:
        match_date = re.search(r"(\d{2})[\/\-](\d{2})[\/\-](\d{4})", text)  # fallback kiểu dd/mm/yyyy
    ...
    if match_date:
        try:
            ...
            parsed["invoice_date"] = None
    else:
        print("❌ Không tìm thấy ngày hóa đơn")
